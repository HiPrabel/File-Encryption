...
public class EncryptorAndDecryptor extends SwingWorker<Boolean, Boolean> {
    ...
    protected Boolean doInBackground() {
        try {
            ...
            new ExceptionDialog("Unexpected System Error!", "Something hugely badly unexpectadly went awfully wrong", e)
                    .setVisible(true);
        } finally {
            return true;
        }
    ...
    private void encrypt(File file) {
        if (!file.isDirectory() && file.exists()) {
            ...
            fileEncryptorAndDecryptor.encrypt(file, key, progressBar, progressPercentLabel, totalSizeOfAllFiles,
            ...
            progressOfFilesTextField
                    .append("~~~~~~~~~~~~~~~~~~~~   End of " + file.getAbsolutePath() + "   ~~~~~~~~~~~~~~~~~~~~\n\n");
        }
    }
    ...
    private void decrypt(File file) {
        if (!file.isDirectory() && file.exists() && file.getName()
                .substring(file.getName().length() - 4, file.getName().length()).equalsIgnoreCase(".enc")) {
            progressOfFilesTextField.append("Decrypting " + file.getAbsolutePath() + "\n");
            fileEncryptorAndDecryptor.decrypt(file, key, progressBar, progressPercentLabel, totalSizeOfAllFiles,
                    progressOfFilesTextField);
            progressOfFilesTextField.append("Done!\n\n");
        } else if (file.isDirectory() && file.exists()) {
            File[] filesInTheDirectory = file.listFiles();
